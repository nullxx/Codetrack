...
const create = async (req, res, next) => {
    const { project, localPath, natures } = req.body;
    ...
    try {
        ...
        if (Array.isArray(data)) {
            createdSnapshot = await createMultipleSnapshots({ fileDatas: data, project, localPath, natures: naturesStr, user: req.user.id });
        } else {
            createdSnapshot = await createSnapshot({ fileData: data, project, localPath, natures: naturesStr, user: req.user.id });
        }
